...
void find_or_insert_safe_ptr(T* table, uint64_t KEY_NUM, K* d_keys, S* d_scores,
                             V* d_vectors, uint64_t dim, hipStream_t& stream) {
  ...
  ROCM_CHECK(hipMalloc(&d_key_ptrs, KEY_NUM * sizeof(K*)));
  ...
  for (int i = 0; i < TEST_TIMES; i++) {
...

    {
                            ...
                            hipMemcpyHostToDevice));
      {
        ...
        ROCM_CHECK(hipMalloc(&d_key_ptrs, BASE_KEY_NUM * sizeof(K*)));
...

    {
...

      {
        ...
        ROCM_CHECK(hipMalloc(&d_key_ptrs, TEST_KEY_NUM * sizeof(K*)));
