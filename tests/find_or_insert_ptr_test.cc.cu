...
void test_rehash_on_big_batch(size_t max_hbm_for_vectors, int key_start = 0) {
...

  {
    V** d_vectors_ptr = nullptr;
    bool* d_found;
    ROCM_CHECK(hipMalloc(&d_found, INIT_KEY_NUM * sizeof(bool)));
    ROCM_CHECK(hipMalloc(&d_vectors_ptr, INIT_KEY_NUM * sizeof(V*)));
  ...
  for (int i = 0; i < TEST_TIMES; i++) {
...

    {
...

      {
        V** d_vectors_ptr = nullptr;
        bool* d_found;
        ROCM_CHECK(hipMalloc(&d_found, BASE_KEY_NUM * sizeof(bool)));
        ROCM_CHECK(hipMalloc(&d_vectors_ptr, BASE_KEY_NUM * sizeof(V*)));
...

    {
...

      {
        V** d_vectors_ptr = nullptr;
        bool* d_found;
        ROCM_CHECK(hipMalloc(&d_found, TEST_KEY_NUM * sizeof(bool)));
        ROCM_CHECK(hipMalloc(&d_vectors_ptr, TEST_KEY_NUM * sizeof(V*)));
    ...
    for (int r = 0; r < rounds; r++) {
      ...
      for (int s = 0; s < STEPS; s++) {
                      ...
                      stream);
        {
          V** d_vectors_ptr = nullptr;
          bool* d_found;
          ROCM_CHECK(hipMalloc(&d_found, BATCH_SIZE * sizeof(bool)));
          ROCM_CHECK(hipMalloc(&d_vectors_ptr, BATCH_SIZE * sizeof(V*)));
...
void CheckFindOrInsertValues(Table* table, K* keys, V* values, S* scores,
...

  {
    V** d_vectors_ptr = nullptr;
    bool* d_found;
    ROCM_CHECK(hipMalloc(&d_found, len * sizeof(bool)));
    ROCM_CHECK(hipMalloc(&d_vectors_ptr, len * sizeof(V*)));
...
void test_duplicated_keys(size_t max_hbm_for_vectors, size_t key_start = 0) {
  ...
  ROCM_CHECK(hipMemset(h_keys, 1, KEY_NUM * sizeof(K)));
  ROCM_CHECK(hipMemset(h_vectors, 0, KEY_NUM * sizeof(V) * options.dim));
