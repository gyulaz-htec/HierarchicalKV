...
void test_basic(size_t max_hbm_for_vectors, int key_start = 0) {
  ...
  ROCM_CHECK(hipMalloc(&d_new_vectors, KEY_NUM * sizeof(V) * options.dim));
  ...
  for (int i = 0; i < TEST_TIMES; i++) {
    ...
    ROCM_CHECK(hipMemset(d_new_vectors, 2, KEY_NUM * sizeof(V) * options.dim));
    ...
    ROCM_CHECK(hipMemset(d_found, 0, KEY_NUM * sizeof(bool)));
    ROCM_CHECK(hipMemset(d_new_vectors, 0, KEY_NUM * sizeof(V) * options.dim));
    {
      V** d_vectors_ptr = nullptr;
      ROCM_CHECK(hipMalloc(&d_vectors_ptr, KEY_NUM * sizeof(V*)));
    ...
    ROCM_CHECK(hipMemcpy(h_found, d_found, KEY_NUM * sizeof(bool),
                          hipMemcpyDeviceToHost));
    ROCM_CHECK(hipMemcpy(h_vectors, d_new_vectors,
                          KEY_NUM * sizeof(V) * options.dim,
                          hipMemcpyDeviceToHost));
    found_num = 0;
    ...
    ROCM_CHECK(hipMemset(d_keys, 0, KEY_NUM * sizeof(K)));
    ROCM_CHECK(hipMemset(d_scores, 0, KEY_NUM * sizeof(S)));
    ROCM_CHECK(hipMemset(d_vectors, 0, KEY_NUM * sizeof(V) * options.dim));
  ...
  ROCM_CHECK(hipFree(d_new_vectors));
...
void test_basic_when_full(size_t max_hbm_for_vectors, int key_start = 0) {
  ...
  for (int i = 0; i < TEST_TIMES; i++) {
    ...
    ROCM_CHECK(hipMemset(d_def_val, 0, KEY_NUM * sizeof(V) * options.dim));
    ...
    ROCM_CHECK(hipMemcpy(h_found, d_found, KEY_NUM * sizeof(bool),
                          hipMemcpyDeviceToHost));
    ROCM_CHECK(hipMemcpy(h_vectors, d_def_val,
                          KEY_NUM * sizeof(V) * options.dim,
                          hipMemcpyDeviceToHost));
    for (int i = 0; i < KEY_NUM; i++) {
      if (h_found[i]) {
...
void test_rehash(size_t max_hbm_for_vectors, int key_start = 0) {
  ...
  for (int i = 0; i < TEST_TIMES; i++) {
    ...
    {
      V** d_vectors_ptr = nullptr;
      ROCM_CHECK(hipMalloc(&d_vectors_ptr, BUCKET_MAX_SIZE * sizeof(V*)));
...
void test_export_batch_if(size_t max_hbm_for_vectors, int key_start = 0) {
  ...
  for (int i = 0; i < TEST_TIMES; i++) {
    ...
    ROCM_CHECK(hipMemset(h_keys, 0, KEY_NUM * sizeof(K)));
    ROCM_CHECK(hipMemset(h_scores, 0, KEY_NUM * sizeof(S)));
    ROCM_CHECK(hipMemset(h_vectors, 0, KEY_NUM * sizeof(V) * options.dim));
...
void test_basic_for_cpu_io(int key_start = 0) {
  ...
  for (int i = 0; i < TEST_TIMES; i++) {
    ...
    ROCM_CHECK(hipMemset(d_vectors, 2, KEY_NUM * sizeof(V) * options.dim));
...
void CheckAssignOnEpochLfu(Table* table,
  ...
  ROCM_CHECK(hipMallocAsync(&d_tmp_founds, cap * sizeof(bool), stream));
  ROCM_CHECK(hipMemsetAsync(d_tmp_founds, 0, cap * sizeof(bool), stream));
  ...
  ROCM_CHECK(hipMemcpyAsync(h_tmp_keys + table_size_before, keys,
                             len * sizeof(K), hipMemcpyDeviceToHost, stream));
  ROCM_CHECK(hipMemcpyAsync(h_tmp_values + table_size_before * dim, values,
                             len * dim * sizeof(V), hipMemcpyDeviceToHost,
                             stream));
  ROCM_CHECK(hipMemcpyAsync(h_tmp_scores + table_size_before, scores,
  ...
  ROCM_CHECK(hipMemcpyAsync(h_tmp_scores, d_tmp_scores,
                             ...
                             hipMemcpyDeviceToHost, stream));
  ROCM_CHECK(hipMemcpyAsync(h_tmp_keys + table_size_after, evicted_keys,
                             filtered_len * sizeof(K), hipMemcpyDeviceToHost,
                             stream));
  ROCM_CHECK(hipMemcpyAsync(h_tmp_values + table_size_after * dim,
                             evicted_values, filtered_len * dim * sizeof(V),
                             hipMemcpyDeviceToHost, stream));
  ROCM_CHECK(hipMemcpyAsync(h_tmp_scores + table_size_after, evicted_scores,
  ...
  ROCM_CHECK(hipFreeAsync(d_tmp_founds, stream));
  free(h_tmp_keys);
  free(h_tmp_values);
  free(h_tmp_scores);
  free(h_tmp_founds);
